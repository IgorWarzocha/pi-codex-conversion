Codex repo contains reference files for comparing the extension.
We are working on a Pi extension that converts Pi to as close as possible to Codex's toolkit.

Release note: do not attempt `npm publish` from the agent. It requires user-controlled npm authentication/2FA and consistently fails in-agent; prepare, validate, commit, tag, and push releases, then tell the user to run `npm publish` locally.
